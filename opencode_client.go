...
// openCodeAssistantError is the typed error returned when an OpenCode
...
type openCodeAssistantError struct {
	detail       string
	statusCode   int
	providerCode string
	name         string
...
func (e *openCodeAssistantError) Error() string {
	if e.detail == "" {
...
const openCodeOutputFormatJSONSchema = "json_schema"
...
func validateOpenCodeStructuredOutputSchema(doc map[string]any) error {
	...
	if !ok || !openAPIStringEnumContains(typeSchema, openCodeOutputFormatJSONSchema) {
		return fmt.Errorf(
