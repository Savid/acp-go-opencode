...
func (h *harness) newSession(opts ...opencodeacp.SessionRequestOption) acp.NewSessionResponse {
	h.t.Helper()

	resp, err := h.conn.NewSession(h.ctx(), opencodeacp.NewSessionRequest(h.t.TempDir(), opts...))
	require.NoError(h.t, err)
...
func (h *harness) prompt(sessionID acp.SessionId, text string, meta map[string]any) (acp.PromptResponse, error) {
	...
	request := opencodeacp.TextPromptRequest(sessionID, text)
	request.Meta = meta
