...
var buildIntegrationBinary = sync.OnceValues(func() (string, error) {
	dir, err := os.MkdirTemp("", "acp-go-opencode-integration-")
	if err != nil {
		return "", err
	}
	binaryBuildDir = dir
	name := "acp-go-opencode"
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, "go", "build", "-o", binary, "./cmd/acp-go-opencode")
...
func integrationBinaryPath(t *testing.T) string {
	t.Helper()
	if binary := os.Getenv("ACP_GO_OPENCODE_AGENT_BINARY"); binary != "" {
