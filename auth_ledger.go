...
var ledgerMarshal = json.Marshal
...
func (l *authLedger) writeEntry(record authLedgerRecord) error {
	...
	return syncLedgerDirectory(l.dir)
}
...
// inventory reports residence from the ledger and a probe of the named native
// slot. The ledger alone is never sufficient — an adapter's record of its own
// intent cannot prove residence — and a probe alone proves only that something
// is resident, not that it is the thing this connection installed.
...
func (p *providerAuth) slotOccupied(ctx context.Context, session *session, providerID string) (bool, error) {
	...
	_, present, err := client.StoredProviderAuth(ctx, providerID)
	if err != nil {
