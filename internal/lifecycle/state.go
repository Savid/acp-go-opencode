...
type State struct {
	...
	// SuppressedRetransmissions counts frames recognized as exact
	// retransmissions of an already-reduced identity.
	...
	// Closed reports that the incarnation is fenced: its descendants are
	// contained and no later event is possible from it.
...
// authoritative fact covers every transition reduced so far; InvalidatedAt
// records the sequence that revoked the last certification.
...
// quiescence: it is the precondition an emitter observes before it may claim
// one.
