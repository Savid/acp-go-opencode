...
// value, and a *ViolationError naming the closed token for every frame it
// refuses.
...
// identity-only session_info_update is the only eligible carrier: it sets no
// title and no updatedAt, so carrying an envelope mutates no state.
...
// ordering identity it managed to decode, so a refusal names the frame it
// refused.
type decoder struct {
...
// version reads the single negotiated integer every envelope and correlation
// value carries.
func (d *decoder) version(fields map[string]json.RawMessage) {
	...
	if version != Version || !d.negotiated.Present() {
		d.fail(ViolationUnsupportedVersion, "version "+strconv.Itoa(version)+" is not exact version 1")
	}
}
...
// watermark fences sequences at or before it, so it can never reach the event
// that carries it.
