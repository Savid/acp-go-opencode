...
// Event is one decoded lifecycle event. Exactly one payload pointer matches
// Type, and the decoder is the only producer, so a reducer never has to defend
// against a discriminant without its payload.
type Event struct {
	Type           EventType
	Snapshot       *Snapshot
...
// Snapshot opens a stream with the whole truth it can state: the foreground
// state and cycle, the complete nonterminal activity and action sets, and the
// current quiescence fact with its proof source.
...
type ActivityUpdate struct {
	...
	// both comparisons the contract makes: the whole-frame fingerprint that
	// suppresses an exact retransmission, and the member-wise restatement check
	// that refuses any difference carried against an already-terminal record.
	Progress json.RawMessage
}

// ActionUpdate reports one permission or elicitation. Only an action blocking
// the current foreground cycle bears on requires_action.
//
// BlocksForeground is a pointer because absence and false are different facts:
// it is required on a first sight, and a later patch that omits it restates
// nothing.
...
type Delivery struct {
	...
	Event    Event
	// Frame is the whole delivered notification as a decoded value — envelope
	// and carrier together. Comparing decoded values is what distinguishes an
	// exact retransmission from a conflicting reuse of the same identity: key
	// order and insignificant whitespace are never differences, and nothing has
	// to retain raw bytes for the life of a session.
	Frame any
}
