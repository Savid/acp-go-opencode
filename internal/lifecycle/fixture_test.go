...
// controlSessionClosed is the whole out-of-band control vocabulary. An input
// carrying only a control is a wire event with no notification of its own.
...
// whole closed vocabulary in both directions: a token with no vector would be a
// rule this reducer could claim without evidence, and a pinned token outside the
// vocabulary would be a verdict no rule here can reach.
func TestFixtureBatteryPinsEveryViolationToken(t *testing.T) {
	...
	for token, file := range pinned {
		require.Contains(t, violationVocabulary, token, "%s pins %s, which is outside the closed vocabulary", file, token)
	}
...
// A control element consumes an index of its own: the refusal index counts every
// input the battery lists, notification or not.
