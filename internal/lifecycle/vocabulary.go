// Package lifecycle implements the `acp-go.dev/lifecycle` extension for this
// adapter: the closed event vocabulary, the strict wire decoder, the ordered
// emitter, and the reducer that validates one session lifecycle stream.
//
// The reducer is deliberately self-contained rather than shared with any other
// implementation. Independently written reducers reaching the same verdict on
// the canonical fixture battery is what proves the wire contract is
// unambiguous; a shared package would prove only that one implementation agrees
// with itself.
//
// It reduces the whole event set, including events this adapter's own
// configuration never emits, because the same code validates the stream this
// adapter publishes: a validator that only handles what the harness happens to
...
const (
	// MetaKey is the family-reserved ACP `_meta` key every lifecycle value
	// rides under. It is a family literal, not a vendor namespace.
	MetaKey = "acp-go.dev/lifecycle"

	// Version is the only envelope version this package implements.
	Version = 1

	// IdentifierBound is the largest opaque identifier the extension carries.
	// An identifier is a correlation handle rather than a payload, so a longer
	// one fails closed.
	IdentifierBound = 4096
)

// ProofClass names how a configuration proves background quiescence. The set is
// closed at one member: quiet time, prompt return, a status poll, process
// silence, queue drainage, a drain window, and ACP idle are never members.
...
// Valid reports whether the proof class is the one member the set has.
...
// where nothing may be delivered between prompts.
