...
func resolveOrdinaryProcessExecutable(path string, environment []string) (string, error) {
	...
	resolve := func(candidate string) (string, error) {
		...
		return windowsExecutableFile(candidate, extensions)
	}
...
func windowsExecutableFile(path string, extensions []string) (string, error) {
	if filepath.Ext(path) != "" {
		if resolved, err := regularOrdinaryExecutableFile(path); err == nil {
			return resolved, nil
		}
	}

	for _, extension := range extensions {
		if resolved, err := regularOrdinaryExecutableFile(path + extension); err == nil {
			return resolved, nil
		}
	}
...
func regularOrdinaryExecutableFile(path string) (string, error) {
