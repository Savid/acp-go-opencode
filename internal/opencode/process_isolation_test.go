...
func TestProcessIsolationValidationAndExecutableResolutionBranches(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, executable, resolved.Path)
...
func TestOrdinaryExecutableResolutionDoesNotInheritPolicyPathRules(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, tool, resolved.Path)
	...
	require.NoError(t, err)
	require.Equal(t, tool, ordinary.Path, "an ordinary relative shape is still anchored before anything can change directory")
...
func TestSupervisorConfigIsInheritedUnlinkedDescriptor(t *testing.T) {
	root := t.TempDir()
	file, err := writeSupervisorConfig(root, supervisorConfig{NativeExecutable: testNativeExecutable(t, "/usr/bin/true"), Home: root, Scratch: root, IsolationUID: 1, IsolationGID: 2})
	require.NoError(t, err)
	t.Cleanup(func() { _ = file.Close() })
