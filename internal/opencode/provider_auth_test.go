...
func TestReapAbandonedHomesRemovesOnlyUnlockedHomes(t *testing.T) {
	parent := t.TempDir()

	abandoned := filepath.Join(parent, "acp-go-opencode-auth-broker-abandoned")
	if err := os.MkdirAll(abandoned, 0o700); err != nil {
		t.Fatalf("create abandoned home: %v", err)
	}
	...
	foreign := filepath.Join(parent, "unrelated")
	if err := os.MkdirAll(foreign, 0o700); err != nil {
		t.Fatalf("create unrelated dir: %v", err)
	}

	if err := os.WriteFile(filepath.Join(parent, "acp-go-opencode-auth-broker-file"), []byte("x"), 0o600); err != nil {
		t.Fatalf("create file: %v", err)
	}

	held, err := reapAcquire(live)
	if err != nil {
		t.Fatalf("hold the live home: %v", err)
	}
	...
	if _, err := os.Stat(abandoned); !errors.Is(err, os.ErrNotExist) {
		t.Fatal("the abandoned home survived the sweep")
	}

	if _, err := os.Stat(live); err != nil {
		t.Fatal("the live home was reclaimed while it was still held")
	}

	if _, err := os.Stat(foreign); err != nil {
		t.Fatal("an unrelated directory was reclaimed")
	}
}
...
func TestReapAbandonedHomesReportsFailures(t *testing.T) {
	...
	parent := t.TempDir()
	if err := os.MkdirAll(filepath.Join(parent, "prefix-one"), 0o700); err != nil {
		t.Fatalf("create home: %v", err)
	}

	originalRemove := reapRemoveAll
	reapRemoveAll = func(string) error { return errors.New("remove") }

	t.Cleanup(func() { reapRemoveAll = originalRemove })
