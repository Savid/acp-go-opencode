	...
	for _, testCase := range []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62609, 62610, "no-domain", "/srv/opencode/no-domain", 1, 2),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovPermanentLock(t, directory, agentStandaloneCovOwnersLock)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62621, 62622, "lockless", "/srv/opencode/lockless", 1, 2),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62623, 62624, "gid-a", "/srv/opencode/gid-a", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62625, 62624, "gid-b", "/srv/opencode/gid-b", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62627, 62628, "shared-owner", "/srv/opencode/owner-a", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62629, 62630, "shared-owner", "/srv/opencode/owner-b", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62631, 62632, "path-a", "/srv/opencode/shared", 1, 2),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62633, 62634, "path-b", "/srv/opencode/shared", 3, 4),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62635, 62636, "inode-a", "/srv/opencode/inode-a", 9, 9),
				)
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62637, 62638, "inode-b", "/srv/opencode/inode-b", 9, 9),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62639, 62640, "anchor", "/srv/opencode/anchor", 1, 2),
				)
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62643, 62644, "bound-gid", "/srv/opencode/bound-gid", 1, 2),
				)
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				...
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62647, 62648, "stale-marker", "/srv/opencode/stale-marker", 1, 2),
				)
...
func TestAgentStandaloneCovAuthorityRootAuditAcceptsACoherentRegistry(t *testing.T) {
	...
	first := agentStandaloneCovOwner(62651, 62652, "coherent-a", "/srv/opencode/coherent-a", 11, 12)
	second := agentStandaloneCovOwner(62653, 62654, "coherent-b", "/srv/opencode/coherent-b", 13, 14)
...
func TestAgentStandaloneCovOwnerUniquenessRefusesEveryCollision(t *testing.T) {
	want := agentStandaloneCovOwner(62501, 62502, "cov-uniqueness", "/srv/opencode/cov-uniqueness", 71, 72)
	suffix := agentStandaloneCovSuffix
	for _, testCase := range []struct {
		name    string
		expired bool
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62501, 62502, "someone-else", "/srv/opencode/someone-else", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62507, 62502, "gid-holder", "/srv/opencode/gid-holder", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62509, 62510, "cov-uniqueness", "/srv/opencode/other-root", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62511, 62512, "path-holder", "/srv/opencode/cov-uniqueness", 81, 82),
				)
			},
		...
		},
		{
			...
			setup: func(t *testing.T, directory *os.File) {
				t.Helper()
				agentStandaloneCovWriteOwner(t, directory,
					agentStandaloneCovOwner(62513, 62514, "inode-holder", "/srv/opencode/other-inode-root", 71, 72),
				)
			},
...
func TestAgentStandaloneCovOwnerUniquenessAcceptsDisjointRegistryState(t *testing.T) {
	...
	agentStandaloneCovWriteOwner(t, directory,
		agentStandaloneCovOwner(62519, 62520, "disjoint", "/srv/opencode/disjoint", 91, 92),
	)
...
func TestAgentStandaloneCovPriorDispositionRequiresTheExactRetainedMarker(t *testing.T) {
	ownerUID, ownerGID := agentStandaloneTestAuthorityIDs()
	owner := agentStandaloneCovOwner(62531, 62532, "prior-disposition", "/srv/opencode/prior-disposition", 51, 52)
...
func TestAgentStandaloneCovOwnerClaimRefusesEveryUnsafeRegistryState(t *testing.T) {
	ownerUID, ownerGID := agentStandaloneTestAuthorityIDs()
	want := agentStandaloneCovOwner(62541, 62542, "cov-claim", "/srv/opencode/cov-claim", 61, 62)

	t.Run("uid bound to another tuple", func(t *testing.T) {
		directory := openAgentStandaloneTestDirectory(t)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62541, 62542, "another-claim", "/srv/opencode/another-claim", 63, 64),
		)
	...
	t.Run("existing binding with a colliding peer", func(t *testing.T) {
		...
		agentStandaloneCovWriteOwner(t, directory, want)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62543, 62542, "gid-collider", "/srv/opencode/gid-collider", 65, 66),
		)
	...
	t.Run("colliding peer blocks a fresh binding", func(t *testing.T) {
		directory := openAgentStandaloneTestDirectory(t)
		agentStandaloneCovWriteOwner(t, directory,
			agentStandaloneCovOwner(62545, 62542, "fresh-collider", "/srv/opencode/fresh-collider", 67, 68),
		)
