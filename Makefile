...
build:
	go build ./...
	go build -o .tmp/acp-go-opencode ./cmd/acp-go-opencode
...
## test-integration-smoke: run integration tests against the installed opencode without spending tokens
test-integration-smoke:
	ACP_GO_OPENCODE_RUN_LIVE_TOKENS=0 ACP_GO_OPENCODE_RUN_INTEGRATION=1 go test -race -count=1 -tags=integration -timeout=300s -v ./integration/...

## test-integration-live: run integration tests that spend model tokens
test-integration-live:
	ACP_GO_OPENCODE_RUN_LIVE_TOKENS=1 ACP_GO_OPENCODE_RUN_INTEGRATION=1 go test -race -count=1 -tags=integration -timeout=900s -v ./integration/...
