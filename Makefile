...
.PHONY: audit build clean coverage-check docs-audit fmt fmt-check help lint modernize-check test test-cross-compile test-integration-cover test-integration-live test-integration-smoke tidy vuln
...
docs-audit:
	...
	@for flag in -path -home -scratch-dir -darwin-best-effort-containment -model -debug -version -seed-file -opencode-pure -opencode-question-tool -opencode-log-level -opencode-health-timeout; do rg -q -- "$$flag" docs/reference/cli.mdx cmd/acp-go-opencode/main.go || { echo "missing CLI flag in docs/code: $$flag"; exit 1; }; done
